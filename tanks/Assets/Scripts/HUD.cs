...
public class HUD : NetworkBehaviour
{
    ...
    private void OnTakeDamage(int side, int damage)
    {
        damageAmount = damage;
        damageSide = side;
    ...
    private void DrawBar(Color c, Vector3 pos, int offsetY, int current, int max)
    {
    ...
    private void DrawArmour(Rect pos, Texture t, float value)
    {
        if (value >= 0.8f)
        {
            GUI.color = Color.green;
