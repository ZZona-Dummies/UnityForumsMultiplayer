...
public class ControlPoint : NetworkBehaviour
{
    ...
    [SyncVar]
    private int owningTeam = -1;

    [SyncVar]
    private int tryingTeam = -1;

    private float scoreTimer;
    private const int controlTime = 10;
    ...
    [Server]
    private void ScoreTickUp(int team, int amount)
    {
        if (!complete)
        {
            if (score == 0)
    ...
    [Server]
    private void ScoreTickDown(int team, int amount)
    {
        score -= amount;
        if (score <= 0)
        {
